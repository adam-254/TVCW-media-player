...
import { Search, Film, Radio, Headphones } from "lucide-react";
    ...
    <header className="fixed top-0 left-0 right-0 z-50 h-16 bg-cyber-dark/80 backdrop-blur-md border-b border-cyber-border/30">
      ...
      <div className="max-w-screen-2xl mx-auto h-full px-3 sm:px-6 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-2 group">
          <Image src="/TVCW-logo.png" alt="TVCW" width={32} height={32}
          ...
          {NAV_LINKS.map(({ href, label, icon: Icon }) => {
            ...
            return (
              <Link key={href} href={href}
                className={cn(
                  "flex items-center gap-2 px-3 sm:px-4 py-2 text-sm font-semibold tracking-wider uppercase transition-all duration-200 font-body hover:text-cyber-cyan",
                  isActive ? "text-cyber-cyan border-b-2 border-cyber-cyan" : "text-cyber-muted hover:text-cyber-cyan"
                )}
              >
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-mono text-cyber-cyan tracking-widest uppercase hidden sm:block">Live</span>
        </div>
      </div>
    </header>
  );
